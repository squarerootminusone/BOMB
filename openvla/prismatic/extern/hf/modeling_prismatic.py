...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action_with_scores(
        self, input_ids: Optional[torch.LongTensor] = None, unnorm_key: Optional[str] = None, **kwargs: str
    ) -> Tuple[np.ndarray, Tuple[torch.FloatTensor, ...]]:
        """Like predict_action(), but also returns per-step score tensors for explainability.

        Returns:
            (actions, scores) where actions is the unnormalized action array and
            scores is a tuple of tensors (one per generated token), each of shape [1, vocab_size].
        """
        ...
        # Decode action (same logic as predict_action)
        predicted_action_token_ids = generated_ids[0, -self.get_action_dim(unnorm_key):].cpu().numpy()
        discretized_actions = self.vocab_size - predicted_action_token_ids
        discretized_actions = np.clip(discretized_actions - 1, a_min=0, a_max=self.bin_centers.shape[0] - 1)
        normalized_actions = self.bin_centers[discretized_actions]
