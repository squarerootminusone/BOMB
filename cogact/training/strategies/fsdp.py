    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            if overwatch.is_rank_zero():
                ...
                torch.save({"model": model_state_dicts, "step": global_step, "epoch": epoch, "loss": train_loss}, latest_path)
                overwatch.info(f"Saved latest checkpoint at step {global_step} to {latest_path}")

                # Track best checkpoint by training loss
                best_path = checkpoint_dir / "best.pt"
                if train_loss is not None:
                    best_loss = float("inf")
                    if best_path.exists():
                        try:
                            best_loss = torch.load(best_path, map_location="cpu").get("loss", float("inf"))
                        except Exception:
                            pass
                    if train_loss < best_loss:
                        import shutil
                        shutil.copy2(latest_path, best_path)
                        overwatch.info(f"  >> New best loss={train_loss:.4f} at step {global_step}")
            dist.barrier()
