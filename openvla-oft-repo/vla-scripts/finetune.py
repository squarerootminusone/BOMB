...
def save_training_checkpoint(
    ...
    Returns:
        None.
    """
    # Always save to latest/ (overwritten each time)
    checkpoint_dir = run_dir / "latest"
    ...
    if distributed_state.is_main_process:
        ...
        print(f"Saving Model Checkpoint for Step {log_step} to {checkpoint_dir}")
    ...
    # Skip LoRA merge during training to save time and disk space.
    # Merge offline after training if needed.
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # Validation + checkpointing (save latest every val_freq, best when improved)
            if cfg.use_val_set and log_step > 0 and log_step % cfg.val_freq == 0:
                ...
                if val_metrics and val_metrics.get("loss", float("inf")) < best_val_loss:
                    ...
                    if distributed_state.is_main_process:
                        ...
                        shutil.copytree(run_dir / "latest", best_dir)
                        print(f"  >> New best val_loss={best_val_loss:.4f} at step {log_step}")
