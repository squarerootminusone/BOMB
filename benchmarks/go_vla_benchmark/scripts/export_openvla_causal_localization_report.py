#!/usr/bin/env python3
"""Export human-readable causal-localization reports from a saved trace."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--trace-input", required=True, type=str, help="causal-localization .npz trace path")
...
def _load_adjusted_clips(args: argparse.Namespace, demo_keys: List[str]):
    trace_bundle = load_causal_trace_file(resolve_repo_relative_path(args.trace_input, repo_root=REPO_ROOT))
...
def main() -> None:
    ...
    manifest = export_causal_localization_report(
