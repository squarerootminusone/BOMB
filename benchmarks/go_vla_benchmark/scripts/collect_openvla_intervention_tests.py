#!/usr/bin/env python3
"""Collect intervention-test traces for OpenVLA checkpoints on Go demos."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-adapter", choices=sorted(INTERVENTION_MODEL_ADAPTERS.keys()), default="openvla")
    ...
    parser.add_argument(
        "--runs",
        type=str,
        default="patch-occlusion,text-masking,minimal-counterfactual",
        help="comma-separated subset of patch-occlusion,text-masking,minimal-counterfactual",
    )
    parser.add_argument("--top-k", type=int, default=8, help="number of top single-edit interventions to store per step")
    parser.add_argument("--max-counterfactual-edits", type=int, default=4, help="greedy counterfactual budget")
...
def _parse_runs(raw: str) -> set[str]:
    ...
    if invalid:
        raise ValueError(f"unsupported runs requested: {invalid}")
...
def main() -> None:
    ...
    traces = collect_episode_intervention_traces(
        ...
        run_patch_occlusion="patch-occlusion" in selected_runs,
        run_text_masking="text-masking" in selected_runs,
        run_counterfactual="minimal-counterfactual" in selected_runs,
        top_k=args.top_k,
        max_counterfactual_edits=args.max_counterfactual_edits,
    )
