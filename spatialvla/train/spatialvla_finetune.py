...
class ModelArguments:
    ...
    mask_rotation_loss: bool = field(
        default=False,
        metadata={
            "help": (
                "Mask loss on rotation action tokens. Enable for datasets with "
                "zero-padded rotation dims (e.g., go_vla_dataset) to prevent "
                "gradient flow through constant-zero rotation dimensions."
            ),
        },
    )
    ...
    dry_run: bool = field(
        default=False,
        metadata={"help": "Use random weights (skip downloading). For testing the training loop."},
    )
...
def main():
    ...
    _processor = SpatialVLAProcessor.from_pretrained(model_args.model_name_or_path, local_files_only=False)
    tokenizer = _processor.tokenizer
    torch_dtype = torch.bfloat16 if training_args.bf16 else torch.float32
    
    logger.info("Loading SpatialVLA Model...")
    config = SpatialVLAConfig.from_pretrained(model_args.model_name_or_path, local_files_only=False)
    config.torch_dtype = torch_dtype
    if model_args.dry_run:
        logger.info("Initializing model with RANDOM weights (dry run)")
        model = SpatialVLAForConditionalGeneration(config)
        model = model.to(torch_dtype)
    else:
        load_kwargs = dict(config=config, torch_dtype=torch_dtype, local_files_only=False)
    ...
    model.mask_rotation_loss = getattr(model_args, "mask_rotation_loss", False)
