...
def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
    ...
    # Mask loss on rotation tokens for datasets with zero-padded rotation dims.
    # When mask_rotation_loss is True, rotation token labels are set to IGNORE_INDEX
    # so CrossEntropyLoss skips them, preventing gradient flow through constant-zero dims.
    _unwrapped = self.accelerator.unwrap_model(model) if hasattr(self, "accelerator") else model
    if getattr(_unwrapped, "mask_rotation_loss", False) and "labels" in inputs:
        _at = getattr(_unwrapped, "action_tokenizer", None)
        if _at is not None:
            rot_start = _at.rotation_tokenizer.token_start_idx
            rot_end = _at.rotation_tokenizer.token_end_idx
            _labels = inputs["labels"]
            rotation_token_mask = (_labels >= rot_start) & (_labels <= rot_end)
            if rotation_token_mask.any():
                inputs["labels"] = _labels.clone()
                inputs["labels"][rotation_token_mask] = IGNORE_INDEX
