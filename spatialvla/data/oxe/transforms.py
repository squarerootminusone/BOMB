...
def go_vla_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    Transform for the Go VLA dataset (5x5 Go board stone placement).

    Why: The raw RLDS actions are 4-DoF [dx, dy, dz, gripper] but SpatialVLA
    requires 7-DoF [dx, dy, dz, roll, pitch, yaw, gripper]. We zero-pad
    the rotation dimensions (indices 3-5) so the action tokenizer receives
    the expected shape. Gripper values are mapped to binary {0, 1}.

    How: Slices the original 4D action into position (0:3) and gripper (3:4),
    inserts three zero columns for rotation, and concatenates to form a 7D vector.
    The language_instruction field is already a top-level step field in the RLDS
    dataset, so no relabeling is needed.
    """
    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, :3],                                    # dx, dy, dz
            tf.zeros_like(trajectory["action"][:, :3]),                     # roll, pitch, yaw (zero-padded)
            binarize_gripper_actions(trajectory["action"][:, 3])[:, None],  # gripper -> {0, 1}
        ],
        axis=-1,
    )
    return trajectory
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    ### Go VLA dataset
    "go_vla_dataset/5.0.0": go_vla_dataset_transform,
}
