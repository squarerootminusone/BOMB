from .mixtures import OXE_NAMED_MIXTURES, OXE_EXCLUDE_MIXTURE
from .configs import OXE_DATASET_CONFIGS, ActionEncoding
from .transforms import OXE_STANDARDIZATION_TRANSFORMS
...
def make_oxe_dataset_kwargs(
    ...
    # Go VLA: 4-DoF zero-padded to 7-DoF; skip normalization on zero-padded rotation dims (3-5)
    if dataset_name.startswith("go_vla_dataset"):
        dataset_kwargs["action_normalization_mask"] = [True, True, True, False, False, False, False]
...
def get_oxe_dataset_kwargs_and_weights(
    ...
    for d_name, d_weight in mixture_spec:
        if d_name in included_datasets:
            print(f"WARNING: Skipping Duplicate Dataset: `{(d_name, d_weight)}`")
            continue
